...
- **`connectConcurrency`** (по умолчанию **3**): сколько профилей подключаются **одной пачкой** (внутри пачки старт сдвигом `connectStaggerMs`). Раньше **1** = строго по одному профилю — очень долго при 10+ окнах.
- **`connectStaggerMs`** (по умолчанию **180**): пауза между стартами соседних профилей в пачке (мс).
- **`maxConcurrentStarts`** (по умолчанию **2**) и **`maxConcurrentStops`** (по умолчанию **1**): раздельные очереди на Dolphin **`/start`** и **`/stop`**. Параллельные **stop** чаще давали **EBUSY**; параллельные **start** по **разным** `profileId` обычно безопаснее и ускоряют первый коннект.
...
- **`autoDiscoverRunningProfileIds: true`** и при этом в конфиге задан **`profileIds`** / **`profileId`**: список **не перезаписывается** автообнаружением (раньше подмена ломала подключение к нужным профилям). Пустой список + auto-discover по-прежнему подставляет id сами.
...
Агент кэширует `login-with-token` (~25 мин). Между стартами соседних профилей в одной пачке используется **`connectStaggerMs`** (по умолчанию 180 ms). При ответе «лимит» делается пауза ~65 с. Не запускайте десятки отдельных процессов агента, каждый из которых долбит API.
