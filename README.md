...
### Сервер в облаке (Railway и т.п.) — ПК не в одной подсети

Если главный компьютер и агенты в **разных сетях** (дом/офис/разные провайдеры), поднимайте sync-сервер **в интернете** (например **Railway**). Тогда не нужны VPN и проброс портов на роутере для синхронизации.

- **WebSocket** в `agent/config.json` и в настройках расширения: **`wss://soft-production-fa8d.up.railway.app/ws`** (именно **`wss://`**, не `ws://`).
- **Панель** в браузере: **`https://soft-production-fa8d.up.railway.app/`** (порт в URL не пишут — его задаёт хостинг).
...
Там же: копирование с главного теперь можно использовать как общий источник текста — сервер также ретранслирует `copy` на агентов.
...
"sync": {
  ...
  "replicateInputValue": true
}
```

- **`replicateInputValue: true`** (по умолчанию): жёстко синхронизирует текст полей по `input` событиям с главного (`value` + позиция каретки). Если нужен только клавиатурный режим (без принудительной подстановки полного значения), поставьте **`false`**.
...
- **Главное окно (расширение)**: копирование/вырезание логируется на сервере и также уходит агентам как событие `copy` (обновляет буфер каждого профиля, чтобы вставка на агентах могла совпадать с главным). Файлы: `clipboard-logs/controller-<controllerId>.log` (рядом с сервером) или путь из переменной окружения **`CLIPBOARD_LOG_DIR`**.
